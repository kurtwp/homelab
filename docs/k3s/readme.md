...
A reference guide for managing your Kubernetes cluster. When using these commands, remember to replace placeholders such as `<ns>`, `<pod>`, and `<name>` with your actual values.
...
* **Node Details**: `kubectl get nodes -o wide` — Lists nodes with extra details including IP, OS, kernel, and roles.
